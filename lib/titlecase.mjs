...
// Title Case heading-style labels that aren't heading tags: FAQ questions, eyebrow labels,
// bold price lines, arrow card links, the hero trust bar and the inline WhatsApp link.
export function titleCaseLabels(html) {
  const tc = (open, inner, close) => open + titleCaseHtml(inner) + close;
  return html
    .replace(/(<summary>)([\s\S]*?)(<\/summary>)/g, (m, o, i, c) => tc(o, i, c))
    .replace(/(<p class="eyebrow">)([\s\S]*?)(<\/p>)/g, (m, o, i, c) => tc(o, i, c))
    .replace(/(<p class="price-from">)([\s\S]*?)(<\/p>)/g, (m, o, i, c) => tc(o, i, c))
    .replace(/(<a class="more"[^>]*>)([\s\S]*?)(<\/a>)/g, (m, o, i, c) => tc(o, i, c))
    .replace(/(<a class="wa-inline"[^>]*>)([\s\S]*?)(<\/a>)/g, (m, o, i, c) => tc(o, i, c))
    .replace(/(<div class="trust">)([\s\S]*?)(<\/div>)/g, (m, o, inner, c) => o + inner.replace(/(<span>)([^<]*)(<\/span>)/g, (x, so, st, sc) => tc(so, st, sc)) + c);
}
